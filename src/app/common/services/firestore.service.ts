...
import { ListaPrecios } from '../models/listaPrecios.models';
...
async createListaPrecios(lista: ListaPrecios): Promise<void> {
  const id = this.createIdDoc(); // Genera un ID único
  lista.fechaCreacion = new Date(); // Asigna fecha de creación
  await this.createDocument<ListaPrecios>(lista, `listasPrecios/${id}`);
}


getListasPrecios(): Observable<ListaPrecios[]> {
  return this.getCollectionChanges<ListaPrecios>('listasPrecios');
}

async getListaPreciosById(id: string): Promise<ListaPrecios | undefined> {
  return await this.getDocumentById<ListaPrecios>('listasPrecios', id);
}

async updateListaPrecios(id: string, lista: Partial<ListaPrecios>): Promise<void> {
  await this.updateDocument<ListaPrecios>(lista, 'listasPrecios', id);
}

async deleteListaPrecios(id: string): Promise<void> {
  await this.deleteDocumentID('listasPrecios', id);
}
